/**
 * Creative Intelligence Suite Documentation Build Pipeline
 *
...
const REPO_URL = 'https://github.com/bmad-code-org/bmad-module-creative-intelligence-suite';
...
// Translated locale directories to exclude from LLM files (only English content is indexed).
// Matches locale keys from website/src/lib/locales.mjs.
const TRANSLATED_LOCALE_DIRS = ['zh-cn'];

// =============================================================================
// Main Entry Point
/**
 * Orchestrates the full Creative Intelligence Suite documentation build pipeline.
 *
...
async function main() {
  console.log();
  printBanner('Creative Intelligence Suite Documentation Build Pipeline');
  console.log();
...
function generateLlmsTxt(outputDir) {
  ...
  const content = [
    ...
    '',
    '> Innovation, brainstorming, and problem-solving agents for the BMad Method.',
    '',
...
function shouldExcludeFromLlm(filePath) {
  ...
  // Exclude translated locale directories (only English content is indexed for LLM consumption)
  if (TRANSLATED_LOCALE_DIRS.some((locale) => filePath.startsWith(`${locale}/`) || filePath.startsWith(`${locale}${path.sep}`)))
    return true;
